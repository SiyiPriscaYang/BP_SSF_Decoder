...
#include "quantum_ldpc_nonbinary_degenerate2.h"
#include <iostream>
#include <sstream>
#include <string>
#include <fstream>
...
//    double th = stod(argv[5]);
//    int job_index = stoi(argv[6]);
int main(int argc, char* argv[]) {
    ...
    string xvname = "xv_" + to_string(job_index) + ".txt";
    ofstream outxv(xvname);

    vector<int> diff(0);
    vector<int> input_x(0);
    int res_type;
    // Loop over trials
    for (int i = 0; i < num_trials; i++) {
        ...
        vector<vector<int> > result= qsc.decode(x,lch,p,num_iter,th);
        diff=result[0];
        input_x=result[1];
        res_type=diff[num_vn];
